...
#'
#' @details This function considers a Leroux et al. (1999) CAR prior (denoted as LCAR) for the spatial latent effects of the different diseases and introduces correlation between them using the M-model proposal of \insertCite{botella2015unifying;textual}{bigDM}.
#' Putting the spatial latent effects for each disease in a matrix, the between disease dependence is introduced through the M matrix as \eqn{\Theta=\Phi M}, where the columns of \eqn{\Phi} follow a LCAR prior distribution (within-disease correlation).
...
Mmodel_lcar <- function(cmd=c("graph","Q","mu","initial","log.norm.const","log.prior","quit"), theta=NULL){
  ...
  Q <- function(){
    ...
    BlockIW <- kronecker(diag(param$alpha),DW) + kronecker(diag(1-param$alpha),Matrix::Diagonal(nrow(W)))
